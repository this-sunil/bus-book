...
const admin = async () => {
  const query = `INSERT INTO users(name,email,phone,pass,role) VALUES($1,$2,$3,$4,$5) RETURNING *`;
...
const createUserTable = async () => {
  const query = `CREATE TABLE IF NOT EXISTS users(id SERIAL PRIMARY KEY,name VARCHAR(255) NOT NULL,phone BIGINT NOT NULL,email VARCHAR(255) NOT NULL,pass VARCHAR(255) NOT NULL,role VARCHAR(255) NOT NULL,created_at DATE DEFAULT CURRENT_DATE)`;
...
export const registerController = async (req, res) => {
  ...
  try {
    ...
    const query =
      "INSERT INTO users(name,email,phone,pass,role) VALUES($1,$2,$3,$4,$5) RETURNING *";
    const { rows } = await pool.query(query, [name, email, phone, hashPass, role]);
     delete rows[0].pass;
    if (rows.length > 0) {
